...
def test_findings_prompt_declares_reviewed_files_when_required() -> None:
    ...
    assert "optional `reviewedFiles`" in without_declaration.output_instructions
...
def test_validate_v2_receipt_preserves_frozen_legacy_findings_contract() -> None:
    # Generated by v2_findings_receipt with FindingsJsonContract from 004b899.
    receipt = json.loads(
...
def test_findings_receipt_rejects_dialect_downgrade_and_identity_mutation(
    ...
    receipt = (
        ...
        if historical
        else v2_findings_receipt()
    )
    if mutation == "digest":
        receipt["attempts"][0]["contractEvaluation"]["preparedSha256"] = "a" * 64
    elif mutation == "unknown-dialect":
        receipt["contract"]["dialect"] = "unknown"
    elif historical:
        receipt["contract"]["dialect"] = "optional-reviewed-files-v1"
    else:
        del receipt["contract"]["dialect"]
    _sign_receipt(receipt)
    assert validate_v2_receipt(receipt)
...
@pytest.mark.parametrize(
    ...
    [
        (
            "findings-json",
            {"name": "findings-json", "version": "1", "dialect": "optional-reviewed-files-v1"},
        ),
...
def test_coverage_matching_rejects_unknown_and_impossible_rules() -> None:
    ...
    # An invalid optional declaration leaves findings missing, without inventing
    # a required reviewedFiles gap.
    assert review_flow_module._coverage_matches_violation(
        "review-declaration",
        ["verdict", "findings"],
        ["verdict"],
        ["findings"],
...
def test_promoted_finding_rejects_prepared_context_exception(monkeypatch) -> None:
    ...
    def fail_prepare(context, digest):
        raise ValueError(digest)

    monkeypatch.setattr(review_flow_module, "resolve_findings_prepared", fail_prepare)
...
def test_consolidate_rejects_hostile_context_and_prepared_exception(monkeypatch) -> None:
    ...
    monkeypatch.setattr(review_flow_module, "findings_contract_for_dialect", fail_prepare)
    assert (
        consolidate(
            None,
            (),
